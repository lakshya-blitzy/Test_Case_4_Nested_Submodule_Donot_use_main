...
`Sensei` result class. Source: runner/mountain.py:L11-L33
...
| Curriculum | `koans/` + `koans.txt` | 39 ordered `TestCase` entries (across 38 lesson modules) that learners complete | Source: koans.txt:L2-L40 |
| Vendored libraries | `libs/colorama`, `libs/mock.py` | Third-party terminal color + mocking, imported but **not modified** here | Source: runner/sensei.py:L14; runner/runner_tests/test_mountain.py:L5 |
...
1. A run surface invokes the launcher — or, for CI, the regression suite.
   `run.sh` runs `python3 -B contemplate_koans.py` (Source: run.sh:L3); `run.bat`
   locates a Windows interpreter and runs `python.exe -B contemplate_koans.py`
   (Source: run.bat:L5, L15-L22); Gitpod auto-runs `python contemplate_koans.py`
   (Source: .gitpod.yml:L5); and Travis CI instead runs the runner-engine
   self-tests via `python _runner_tests.py` (Source: .travis.yml:L7). See the
   [Deployment Guide](./deployment.md) for every entry point.
...
3. `Mountain.__init__` builds the `WritelnDecorator` output stream, loads the
   ...
   `Sensei` bound to that stream. Source: runner/mountain.py:L26-L33
...
5. `walk_the_path` runs the suite against the `Sensei`, then calls
   ...
   runner/mountain.py:L35-L60; runner/sensei.py:L17
...
flowchart TD
    ...
    D --> E["koans.txt<br/>39 TestCase entries"]
    E --> F["Sensei<br/>colored progress + Zen"]
```

*Source: contemplate_koans.py:L31-L61, runner/mountain.py:L11-L60, runner/path_to_enlightenment.py:L92-L109, runner/sensei.py:L17*

## Design Notes

- **Standard-library only.** The runner is built on `unittest` with no runtime
  dependencies beyond the vendored `libs/` (e.g. `colorama` for colored output).
  Source: runner/mountain.py:L4; runner/sensei.py:L14
- **Ordered curriculum.** `koans.txt` lists 39 ordered `TestCase` entries across
  38 lesson modules (`about_proxy_object_project` contributes two), and the loader
  preserves that manifest order when assembling the suite (Source: koans.txt:L2-L40;
  runner/path_to_enlightenment.py:L64-L89, L92-L109). At runtime `Sensei` counts
  304 individual koans — `total_koans` returns `self.tests.countTestCases()`
  (Source: runner/sensei.py:L456-L464) — across 37 standard lessons, since
  `total_lessons` counts `filter_all_lessons`, which globs `koans/about*.py` and
  excludes `about_extra_credit` (Source: runner/sensei.py:L442-L454, L466-L481).
